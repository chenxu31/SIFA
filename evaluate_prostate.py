...
import common_net
import common_pelvic as common_prostate
import common_metrics


def main(args):
    _, test_data, _, test_label = common_prostate.load_test_data(args.data_dir)
    ...
    for c in range(args.num_classes - 1):
        name = common_prostate.MAP_ID_NAME[c + 1]
        dsc = dsc_list[:, c]
        assd = assd_list[:, c]
    ...
    if args.output_dir:
        ...
        for i,im in enumerate(pred_list):
            common_prostate.save_nii(im, os.path.join(args.output_dir, "pred_%d.nii.gz" % (i + 1)))
