...
def pad(image, view, pad_value=-1):
    if view == "axial":
        if image.ndim == 4:
            image = np.pad(image.transpose((0, 1, 3, 2)), ((0, 0), (0, 0), (1, 0), (2, 2)), 'constant', constant_values=pad_value).transpose((0, 2, 3, 1))
        else:
            image = np.pad(image.transpose((0, 2, 1)), ((0, 0), (1, 0), (2, 2)), 'constant', constant_values=pad_value).transpose((1, 2, 0))
    elif view == "coronal":
        if image.ndim == 4:
            image = np.pad(image.transpose((0, 3, 1, 2))[:, :, ::-1, :], ((0, 0), (0, 0), (3, 3), (2, 2)), 'constant', constant_values=pad_value).transpose((0, 2, 3, 1))
        else:
            image = np.pad(image.transpose((2, 0, 1))[:, ::-1, :], ((0, 0), (3, 3), (2, 2)), 'constant', constant_values=pad_value).transpose((1, 2, 0))
    elif view == "sagittal":
        if image.ndim == 4:
            image = np.pad(image.transpose((0, 2, 1, 3))[:, :, ::-1, :], ((0, 0), (0, 0), (3, 3), (1, 0)), 'constant', constant_values=pad_value).transpose((0, 2, 3, 1))
        else:
            image = np.pad(image.transpose((1, 0, 2))[:, ::-1, :], ((0, 0), (3, 3), (1, 0)), 'constant', constant_values=pad_value).transpose((1, 2, 0))
    else:
        assert 0
...
class DataIterBase(object):
    def __init__(self, data_dir, batch_size, view, num_threads=4, seed=123456):
        ...
        self.data_height = self.data_seg.shape[1]
        self.data_width = self.data_seg.shape[2]
        self.data_depth = self.data_seg.shape[3]
        self.patch_height = self.data_height
        self.patch_width = self.data_width

        self.q = Queue.Queue(10)
        self.threads = [threading.Thread(target=self.read_worker) for i in range(num_threads)]
    ...
    def load_data(self, data_dir):
        paired_data = h5py.File(os.path.join(data_dir, "paired_mri_ct.h5"), "r")
        self.data_mri = pad(np.array(paired_data["mri"], np.float32), self.view)
        self.data_ct = pad(np.array(paired_data["ct"], np.float32), self.view)
        self.data_seg = pad(np.array(paired_data["seg"], np.float32), self.view, 0)

        self.raw_data_height, self.raw_data_width, self.raw_data_depth = self.data_mri.shape[1:]
        paired_data.close()
    ...
    def get_shapes(self):
        return {
            ...
            "raw_data_height": self.raw_data_height,
            "raw_data_width": self.raw_data_width,
            "raw_data_depth": self.raw_data_depth,
...
# data iterator for training
class DataIterUnpaired(DataIterBase):
    def load_data(self, data_dir):
        super(DataIterUnpaired, self).load_data(data_dir)

        unpaired_data_mri = h5py.File(os.path.join(data_dir, "unpaired_mri.h5"), "r")
        unpaired_data_ct = h5py.File(os.path.join(data_dir, "unpaired_ct.h5"), "r")
        self.data_unpaired_mri = pad(np.array(unpaired_data_mri["mri"], np.float32), self.view)
        self.data_unpaired_ct = pad(np.array(unpaired_data_ct["ct"], np.float32), self.view)
        self.data_unpaired_seg = pad(np.array(unpaired_data_ct["seg"], np.float32), self.view, 0)

        self.unpaired_mri_subjects = list(range(self.data_unpaired_mri.shape[0]))
        self.unpaired_ct_subjects = list(range(self.data_unpaired_ct.shape[0]))
        unpaired_data_mri.close()
        unpaired_data_ct.close()

    def next_indices(self):
        mri_subjects = [None] * self.batch_size
        ct_subjects = [None] * self.batch_size
        mri_indices = [None] * self.batch_size
        ct_indices = [None] * self.batch_size
        for i in range(self.batch_size):
            mri_subjects[i] = self.unpaired_mri_subjects[random.randint(0, len(self.unpaired_mri_subjects) - 1)]
            ct_subjects[i] = self.unpaired_ct_subjects[random.randint(0, len(self.unpaired_ct_subjects) - 1)]

            height_id = random.randint(0, self.data_height - self.patch_height)
            width_id = random.randint(0, self.data_width - self.patch_width)
            depth_id = random.randint(0, self.data_depth - self.patch_depth)
            mri_indices[i] = (height_id, width_id, depth_id)

            height_id = random.randint(0, self.data_height - self.patch_height)
            width_id = random.randint(0, self.data_width - self.patch_width)
            depth_id = random.randint(0, self.data_depth - self.patch_depth)
            ct_indices[i] = (height_id, width_id, depth_id)

        return mri_subjects, ct_subjects, mri_indices, ct_indices

    def read_worker(self):
        while True:
            # obtain indices of current batch
            mri_subjects, ct_subjects, mri_indices, ct_indices = self.next_indices()

            mri_patch = np.zeros((len(mri_indices), self.patch_height, self.patch_width, self.patch_depth), np.float32)
            ct_patch = np.zeros((len(ct_indices), self.patch_height, self.patch_width, self.patch_depth), np.float32)
            seg_patch = np.zeros((len(ct_indices), self.patch_height, self.patch_width, self.patch_depth), np.uint8)
            for i,item in enumerate(mri_indices):
                mri = self.data_unpaired_mri[mri_subjects[i],
                                             item[0]: item[0] + self.patch_height,
                                             item[1]: item[1] + self.patch_width,
                                             item[2]: item[2] + self.patch_depth]
                ct = self.data_unpaired_ct[ct_subjects[i],
                                           ct_indices[i][0]: ct_indices[i][0] + self.patch_height,
                                           ct_indices[i][1]: ct_indices[i][1] + self.patch_width,
                                           ct_indices[i][2]: ct_indices[i][2] + self.patch_depth]
                seg = self.data_unpaired_seg[ct_subjects[i],
                                             ct_indices[i][0]: ct_indices[i][0] + self.patch_height,
                                             ct_indices[i][1]: ct_indices[i][1] + self.patch_width,
                                             ct_indices[i][2]: ct_indices[i][2] + self.patch_depth]
            ...
            self.q.put((mri_patch, ct_patch, seg_patch))


def generate_image(sess, common_feed, input_op, output_op, input_data, patch_shape, is_seg=False, batch_size=4, num_classes=2):
    if not isinstance(input_op, tuple) and not isinstance(input_op, list):
        input_op = [input_op, ]
    if not isinstance(output_op, tuple) and not isinstance(output_op, list):
        output_op = [output_op, ]
    if not isinstance(input_data, tuple) and not isinstance(input_data, list):
        input_data = [input_data, ]

    assert len(input_op) == len(input_data)

    patch_height, patch_width, patch_depth = patch_shape

    # the shape of all input datas should be the same
    for i in range(1, len(input_data)):
        if (not isinstance(input_data[i], str)) and (not isinstance(input_data[i], list)) and (not isinstance(input_data[i], tuple)):
            assert cmp(input_data[i].shape, input_data[0].shape) == 0
    data_shape = input_data[0].shape

    data_fake = []
    used = []
    batch_input = []
    for i in range(len(input_op)):
        batch_input.append(np.zeros([batch_size,] + [x.value for x in input_op[i].shape[1:]], np.float32))
    for i in range(len(output_op)):
        if is_seg:
            data_fake.append(np.zeros((data_shape[0], data_shape[1], data_shape[2], num_classes), np.float32))
        else:
            data_fake.append(np.zeros(data_shape, np.float32))
        used.append(np.zeros(data_shape, np.float32))

    idx = 0
    batch_locs = [None] * batch_size
    pdb.set_trace()
    for i in range(0, data_shape[2] - patch_depth + 1):
        batch_locs[idx] = i
        for ii,data in enumerate(input_data):
            if isinstance(data, str) and data == "normal":
                batch_input[ii][idx] = np.random.normal(0, 1, (NOISE_LENGTH, ))
            elif (isinstance(data, list) or isinstance(data, tuple)) and data[0] == "constant":
                batch_input[ii][idx] = data[1]
            else:
                batch_input[ii][idx] = data[:, :, i: i + patch_depth]
        ...
        idx = 0
        feed_dict = common_feed
        for ii in range(len(input_data)):
            feed_dict[input_op[ii]] = batch_input[ii]
        ret = sess.run(output_op, feed_dict=feed_dict)

        for batch_id, loc in enumerate(batch_locs):
            for ii in range(len(output_op)):
                if is_seg:
                    data_fake[ii][:, :, loc + patch_depth // 2, :] += ret[ii][batch_id]
                    used[ii][:, :, loc + patch_depth // 2] += 1.
                else:
                    data_fake[ii][:, :, loc: loc + patch_depth] += ret[ii][batch_id]
    ...
    if idx != 0:
        ...
        for batch_id in range(idx):
            loc = batch_locs[batch_id]
            for ii in range(len(output_op)):
                if is_seg:
                    data_fake[ii][:, :, loc + patch_depth // 2, :] += ret[ii][batch_id]
                    used[ii][:, :, loc + patch_depth // 2] += 1.
    ...
    for ii in range(len(data_fake)):
        if is_seg:
            data_fake[ii] = data_fake[ii] / np.expand_dims(used[ii], -1)
        else:
            data_fake[ii] = data_fake[ii] / used[ii]
...
class SIFA:
    ...
    def __init__(self, args):
        ...
        self.data_iter = DataIterUnpaired(args.data_dir, args.batch_size, args.view)
    ...
    def train(self):
        ...
        with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
            ...
            for i in range(self._max_step):
                ...
                mri, ct, seg = self.data_iter.next()

                inputs = {
                    'images_i': ct,
                    'images_j': mri,
                    'gts_i': _label_decomp(self._num_cls, seg),
                    'gts_j': _label_decomp(self._num_cls, seg),
                }
                ...
                if (i + 1) % self._evaluation_interval == 0:
                    ...
                    dsc_mean, dsc_std, assd_mean, assd_std = self.validate(sess)
                    logging.info("Validation (%d) dsc:%f/%f  assd:%f/%f"% (i + 1, dsc_mean, dsc_std, assd_mean, assd_std))
    ...
    def validate(self, sess):
        ...
        for i in range(self.data_iter.data_mri.shape[0]):
            mri_seg = generate_image(sess, common_feed, self.input_b, self.predicter_b, self.data_iter.data_mri[i],
                                     patch_shape, batch_size=self._batch_size, is_seg=True)
            mri_seg = mri_seg.argmax(-1).astype(np.float32)

            mri_dsc = common_metrics.dc(self.data_iter.data_seg[i], mri_seg)
            mri_assd = common_metrics.assd(self.data_iter.data_seg[i], mri_seg)
        ...
        return mri_dsc_list.mean(), mri_dsc_list.std(), mri_assd_list.mean(), mri_assd_list.std()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_dir', type=str, default=r'D:\datasets\mri_seg_disentangle', help='path of the dataset')
    parser.add_argument('--output_root_dir', type=str, default=r'D:\training\logs\SIFA', help='output path')
    parser.add_argument('--checkpoint_dir', type=str, default=r'D:\training\checkpoints\SIFA', help='checkpoint path')
